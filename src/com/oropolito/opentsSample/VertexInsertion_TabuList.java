 ...
 * @since 1.0-exp9
 */
public class VertexInsertion_TabuList implements TabuList
{
    /**
    ...
    * The value 2 will be used as the number of attributes if
     ...
     * @since 1.0-exp9
     */
    public final static int DEFAULT_NUM_ATTR = 5;
     ...
     * @since 1.0-exp3
     */
    public VertexInsertion_TabuList()
    {   this( DEFAULT_TENURE );        
    }   // end SimpleTabuList
     ...
     * @since 1.0-exp3
     */
    public VertexInsertion_TabuList( int tenure)
    {
        super();
        
        this.tenure     = tenure;
        this.numAttr    = DEFAULT_NUM_ATTR;
     ...
     // See if attributes are tabu
        for( int i = 1; i <= tenure; i++ )
            if ( currentPos - i < 0 )
        		 ...
        		 * Per ora solo se prova ad aggiungere edge che era stato rimosso
        		 */
            	//---t1---c1---t2-----------------t3----new_pos---t4--------
            	//Edge rimossi: t1-c1,  c1-t2 , t3-t4
            	//Edge aggiunti: t1-t2, t3-c1, c1-t4
            	int c1 = attrs[0], t1 = attrs[1], t2 = attrs[2], t3 = attrs[3], t4 = attrs[4];
            	int old_c1 = tabuList[ (currentPos-i) % listLength ][0];
            	int old_t1 = tabuList[ (currentPos-i) % listLength ][1];
            	int old_t2 = tabuList[ (currentPos-i) % listLength ][2];
            	int old_t3 = tabuList[ (currentPos-i) % listLength ][3];
            	int old_t4 = tabuList[ (currentPos-i) % listLength ][4];
            	if (isSameEdge(t1, t2, old_t1, old_c1)) return true;
            	if (isSameEdge(t1, t2, old_t2, old_c1)) return true;
            	if (isSameEdge(t1, t2, old_t3, old_t4)) return true;
            	if (isSameEdge(t3, c1, old_t1, old_c1)) return true;
            	if (isSameEdge(t3, c1, old_t2, old_c1)) return true;
            	if (isSameEdge(t3, c1, old_t3, old_t4)) return true;
            	if (isSameEdge(c1, t4, old_t1, old_c1)) return true;
            	if (isSameEdge(c1, t4, old_t2, old_c1)) return true;
            	if (isSameEdge(c1, t4, old_t3, old_t4)) return true;
            }
